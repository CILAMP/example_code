...
.SYNOPSIS
Set the color and pulse attributes of a CILamp with a known service-ID.

.DESCRIPTION
The script accepts (and requires) three parameters:
-serviceId (The ID that you give to your CILamp during the configuration phase. This id is used so that your CILamp knows what service it should listen to.)
...
.EXAMPLE 
./set_cilamp_color.ps1 -serviceId <put the service id here> -color 00ff00 -duration 0
...
$requestURL = "https://api.cilamp.se/v1/" + $serviceId
